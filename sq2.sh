...
sqlite3 data/biopics.sqlite ' SELECT subject_sex as gender, SUM(box_office) FROM (SELECT DISTINCT title, box_office, subject_sex from biopics WHERE subject_sex = "Male" ) GROUP BY gender' | sed 's/^/<tr><td>/g'  |sed 's/|/<\/td><td>$/g' | sed 's/$/<\/td><\/tr>/g' >> output/sa2.html

# Female Query Cmd
# Step 1: Query Distinct Movie title which subject sex = Female
# Step 2: SUM the box office by subject sex
sqlite3 data/biopics.sqlite ' SELECT subject_sex as gender, SUM(box_office) FROM (SELECT DISTINCT title, box_office, subject_sex from biopics WHERE subject_sex = "Female" ) GROUP BY gender' | sed 's/^/<tr><td>/g'  |sed 's/|/<\/td><td>$/g' | sed 's/$/<\/td><\/tr>/g'>> output/sa2.html
