...
grep Male data/biopics.csv | cut -f 1,5,13 -d , | uniq | cut -f 2,3 -d, | sed 's/-/0/g' | awk ' BEGIN { OFS = ","; FS="," } {a += $1} END{print $2, a}' | sed 's/^/<tr><td>/g'  |sed 's/,/<\/td><td>$/g' | sed 's/$/<\/td><\/tr>/g' >> output/ba2.html
...
# STEP 5: Change all the Unknown box office record to 0 (- to 0)
# STEP 6: AWK cmd to summarise the result
grep Female data/biopics.csv | cut -f 1,5,13 -d , | uniq | cut -f 2,3 -d, | sed 's/-/0/g' | awk ' BEGIN { OFS = ","; FS="," } {a += $1} END{print $2, a}' | sed 's/^/<tr><td>/g'  |sed 's/,/<\/td><td>$/g' | sed 's/$/<\/td><\/tr>/g' >> output/ba2.html
