...
sqlite3 data/biopics.sqlite ' SELECT year_release as Year, SUM(box_office) as total_amount FROM biopics GROUP BY Year' | sed 's/^/<tr><td>/g'  |sed 's/|/<\/td><td>$/g' | sed 's/$/<\/td><\/tr>/g' >> output/sa3.html
